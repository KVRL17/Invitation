  ...
  return (
    <section className="relative overflow-hidden px-5 py-14 sm:py-24" aria-label="Countdown to the engagement">
      ...
      <Reveal delay={0.15} className="mt-8 sm:mt-12">
        <div
          className="mx-auto grid max-w-3xl grid-cols-4 gap-2 sm:gap-8"
          role="timer"
          aria-live="polite"
        ...
        >
          {units.map(({ key, label }) => {
            ...
            return (
              <div
                key={key}
                className="glass-card flex flex-col items-center rounded-full border-gold/30 px-0.5 py-4 sm:px-1 sm:py-9"
                style={{ aspectRatio: '1 / 1' }}
              >
                <motion.span
                  key={value}
                  className="font-serif text-2xl font-semibold text-burgundy sm:text-5xl md:text-6xl"
                ...
                <span className="mt-1 font-sans text-[9px] uppercase tracking-[0.28em] text-ink-soft sm:mt-2 sm:text-xs">
                  {label}
                </span>
              </div>
            );
